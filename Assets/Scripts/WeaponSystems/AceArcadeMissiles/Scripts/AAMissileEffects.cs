...
public class AAMissileEffects : MonoBehaviour
{
    ...
    [Tooltip("VFX played at missile's attach point.")]
    ...
    private void Start()
    {
        ...
        if (trailPrefab != null || particleTrailPrefab != null || vfxTrailPrefab != null)
        {
            ...
            // Instantiate the VFX Graph trail.
            if (vfxTrailPrefab != null)
            {
                vfxTrail = Instantiate(vfxTrailPrefab, trailFxPoint);
                vfxTrail.transform.localPosition = Vector3.zero;
                vfxTrail.transform.localEulerAngles = Vector3.zero;

                vfxTrail.Stop();

                // Ensure that the effect remover is on the gameobject to prevent undeleted particle effects.
                if (vfxTrail.GetComponent<AARemoveEffect>() == null)
                {

                }
                    //effectRemover = vfxTrail.gameObject.AddComponent<AARemoveEffect>();
            }
        }
    }
