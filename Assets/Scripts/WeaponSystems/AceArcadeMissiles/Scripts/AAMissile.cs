...
public class AAMissile : MonoBehaviour
{
    ...
    [Tooltip("Damage dealt to the object when the missile hits something;.")]
    ...
    private void OnCollisionEnter(Collision collision)
    {
        ...
        if (isLaunched && TimeSince(launchTime) > dropDelay)
        {
            ...
            if(collision.collider.GetComponent<DamageHandler>() != null)
                collision.collider.GetComponent<DamageHandler>().TakeDamage(missileDamage, shooter, this.gameObject);
            // Destroy the missile
            DestroyMissile(true);
        }
    }
